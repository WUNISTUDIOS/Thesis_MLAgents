...
#region Methods
        ...
        private void RunPrefabPaint()
        {
            ...
            bool rayResult = Physics.Raycast(ray, out hit);

            switch (activeSave.paintType)
            {
                case PB_PaintType.Surface:
                    if (mouseDrag || mouseDown && rayResult)
                        RunSurfacePaint(rayResult, mouseDown, hit, ray, false);
                    break;
                case PB_PaintType.Physics:
                    if (mouseDrag || mouseDown && rayResult)
                        RunSurfacePaint(rayResult, mouseDown, hit, ray, true);
                    break;
                case PB_PaintType.Single:
                    RunSinglePaint(rayResult, mouseDown, mouseDrag, mouseUp, mouseLeaveWindow, hit, ray);
                    break;
            }
        }

        private void RunSurfacePaint(bool rayResult, bool mouseDown, RaycastHit hit, Ray ray, bool physicsBrush)
        {
        ...
        private void RunSinglePaint(bool rayResult, bool mouseDown, bool mouseDrag, bool mouseUp, bool mouseLeaveWindow, RaycastHit hit, Ray ray)
        {
